...
with open(os.path.join('result', 'Tech_superStocks_1_15_2021.json')) as f:
    data = js.load(f)
print(data)
tickers = data['data'][0]
...
for ticker in tickers:
    try:
        ...
        s = {ticker:{'market cap':mcap, 'value':price[0], 'price':stickerPrice, 'current EPS': cEPS, 'book value anual': bv, 'book value growth': growth,
             'book value quater': bvq, 'book value quater growth': qgrowth, 'roic': roic, 'cashflow':cashflow, 'priceSales':priceSales}}    
        ...
        for li in lis:
            ...
            news['news'].update({ff.get_text():{'title':title}})
            news['news'][ff.get_text()].update({'content':content})
            count = count + 1
        ...
        print("error!")
        pass
    jsfile = os.path.join('result', 'detailed_study.json')
